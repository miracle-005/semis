...
def fetch_messages(request, username):
    ...
    try:
        ...
        # ✅ Correct way to fetch the chat room
        ...
        if not messages.exists():
            logger.info(f"🚫 No messages found for chat room {room.id}")

        # ✅ Log messages
        logger.info(f"💬 DEBUG: Messages found: {messages.count()}")
        for msg in messages:
            logger.info(f"📝 {msg.sender}: {msg.message} ({msg.timestamp})")
...
def send_message(request, username):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            # Save message to DB
            new_message = ChatMessage.objects.create(room=room, sender=request.user, message=message_text)
            print(new_message)
